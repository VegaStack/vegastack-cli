...
import { handleRegistryGetRecipe, parseRecipeToml } from "../../src/tools/registry_get_recipe.js";
import { makeEnv, SEED_BASIC } from "../_fixtures.js";

afterEach(() => clearRegistryCaches());

describe("registry_get_recipe", () => {
  it("parses providers, triggers, scaffold, pitfalls", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleRegistryGetRecipe(env, { id: "scalable-backend-aws-ecs-fargate" });
  ...
  it("returns error envelope for missing recipe", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleRegistryGetRecipe(env, { id: "nope" });
    const parsed = JSON.parse(out.content[0]!.text);
