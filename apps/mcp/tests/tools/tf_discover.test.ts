...
import { handleTfDiscover, tfDiscoverSchema } from "../../src/tools/tf_discover.js";
import { makeEnv, SEED_BASIC } from "../_fixtures.js";

afterEach(() => clearBundleCaches());

describe("tf_discover", () => {
  it("schema fields are present and validate basic input", async () => {
    expect(tfDiscoverSchema.query).toBeDefined();
    expect(tfDiscoverSchema.provider).toBeDefined();
    expect(tfDiscoverSchema.max).toBeDefined();
    // empty query rejected
    const parsed = tfDiscoverSchema.query.safeParse("");
    expect(parsed.success).toBe(false);
  });

  it("returns canonical envelope (status='ok') with explicit provider", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfDiscover(env, {
    ...
    expect(top.path).toMatch(/^cli\/bundle\/aws\//);
  ...
  it("auto-detects provider from query when omitted", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfDiscover(env, { query: "cloudflare DNS record" });
    const parsed = JSON.parse(out.content[0]!.text);
    expect(parsed.status).toBe("ok");
  ...
  it("returns error envelope for unknown provider", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfDiscover(env, { query: "anything", provider: "fake-cloud" });
    const parsed = JSON.parse(out.content[0]!.text);
  ...
  it("returns error envelope when no provider can be detected", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfDiscover(env, { query: "make a thing" });
    const parsed = JSON.parse(out.content[0]!.text);
    expect(parsed.status).toBe("error");
    expect(parsed.code).toBe("ProviderUnknown");
  });
  ...
  it("structuredContent matches the JSON-encoded text body", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfDiscover(env, { query: "S3 bucket", provider: "aws" });
