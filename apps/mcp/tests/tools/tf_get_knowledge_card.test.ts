...
describe("tf_get_knowledge_card", () => {
  ...
  it("returns error envelope for missing card", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfGetKnowledgeCard(env, { id: "does-not-exist" });
    const parsed = JSON.parse(out.content[0]!.text);
  ...
  it("rejects malformed ids before fetching", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfGetKnowledgeCard(env, { id: "../etc/passwd" } as { id: string });
    const parsed = JSON.parse(out.content[0]!.text);
    expect(parsed.status).toBe("error");
