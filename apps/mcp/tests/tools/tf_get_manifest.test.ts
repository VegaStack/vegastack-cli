...
describe("tf_get_manifest", () => {
  ...
  it("returns the single resource entry when resource is given", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfGetManifest(env, { provider: "aws", resource: "aws_s3_bucket" });
    const parsed = JSON.parse(out.content[0]!.text);
    expect(parsed.provider).toBe("aws");
  ...
  it("returns error envelope for unknown resource", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfGetManifest(env, { provider: "aws", resource: "aws_does_not_exist" });
    const parsed = JSON.parse(out.content[0]!.text);
  ...
  it("returns error envelope for unknown provider", async () => {
    const env = makeEnv(SEED_BASIC);
    const out = await handleTfGetManifest(env, { provider: "fake" });
    const parsed = JSON.parse(out.content[0]!.text);
    expect(parsed.status).toBe("error");
  });
});
