// MCP tool: tf_get_recipe — fetch a single curated recipe by id.
// Recipes live in `cli/bundle/recipes/<id>.toml` (see docs/contracts/format-examples.md).
...
export const tfGetRecipeSchema = {
  id: z
    .string()
    .regex(/^[a-z0-9][a-z0-9-]*$/)
...
export const tfGetRecipeDescription =
  "Fetch a single curated recipe by id. Recipes are full HCL scaffolds with pitfalls and " +
  "trigger phrases. Use this when tf_discover surfaced a recipe id and you want the full body.";

export type TfGetRecipeArgs = { id: string };

export async function handleTfGetRecipe(env: Env, args: TfGetRecipeArgs) {
  ...
  const key = `cli/bundle/recipes/${args.id}.toml`;
  let raw: string;
  try {
...
// We deliberately do NOT bundle @iarna/toml into the Worker.
