// MCP tool: registry_get_knowledge_card — fetch a single curated knowledge card.
// Knowledge cards live in `cli/packs/terraform/docs/knowledge/<id>.md` with YAML
// frontmatter (see docs/contracts/format-examples.md). The Registry publishing
// workflow uploads each card as an individual R2 object.
//
// We do NOT ship a YAML parser into the worker — frontmatter is parsed by
...
export const registryGetKnowledgeCardSchema = {
  id: z
    .string()
    .regex(/^[a-z0-9][a-z0-9-]*$/)
...
export const registryGetKnowledgeCardDescription =
...
export type RegistryGetKnowledgeCardArgs = { id: string };

export async function handleRegistryGetKnowledgeCard(env: Env, args: RegistryGetKnowledgeCardArgs) {
  ...
  const key = `cli/packs/terraform/docs/knowledge/${args.id}.md`;
  let raw: string;
  try {
    ...
    console.error("registry_get_knowledge_card: internal error", {
      id: args.id,
      message: e instanceof Error ? e.message : String(e),
    });
    return jsonContent({
      status: "error",
      error: "internal error fetching knowledge card",
      code: "Internal",
    });
  }
