...
//
// Reads a single canonical SKILL.md template from `registry/skill-source/` and
...
const TEMPLATE_PATH = path.join(REPO_ROOT, "registry", "skill-source", "SKILL.md.template");
...
export function resolveRegistrySummary(manifestPath = REGISTRY_MANIFEST): RegistrySummary {
  ...
  try {
    ...
    const registryVersion =
      typeof json.pack_version === "string"
        ? json.pack_version
        : typeof json.registry_version === "string"
          ? json.registry_version
          : "dev";
...
export function renderSkill(template: string, summary: RegistrySummary): string {
...
export function runCli(argv = process.argv.slice(2)): number {
  ...
  const summary = resolveRegistrySummary(opts.manifestPath);
  ...
  if (opts.check) {
    ...
    if (onDisk === rendered) {
      process.stderr.write(`ok: ${opts.out} matches the template render\n`);
      return 0;
    }
