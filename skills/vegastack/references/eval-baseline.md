...
1. **Baseline run.** The model configured in `runner.ts:DEFAULT_MODEL` is given the user prompt and a generic system prompt. No `vegastack ask` available, no skills directory, no MCP. Output is scored against the prompt's `expectations[]` by an LLM-as-judge (yes/no per expectation).
...
The eval suite is partitioned by archetype. Treat `evals/evals.json` as the
source of truth for the current prompt list and per-archetype counts.
...
- **Nightly cron** (`cli/.github/workflows/evals.yml`): full suite run on `main`. Artifacts pushed to `vegastack/eval-history` branch and surfaced on the dashboard.
- **Per-PR smoke** (touching `src/lib/discover/**`, `src/lib/generic-pack-discover.ts`, `src/lib/registry.ts`, or `evals/**`): smoke suite for priority archetypes. Blocks merge if lift drops beyond the configured threshold vs. the rolling window on `main`.
- **Release tag**: full suite run gates the npm publish.
...
3. **Archetype imbalance** — archetypes can have different prompt counts. Mitigation: dashboard reports both macro-average and macro-median; per-archetype bars are the primary view.
