...
1. Run `vegastack init --dry-run --json`.
2. Summarize detected stack, selected Registry entries, scan checks, agent hosts, and planned files.
...
4. Run the CLI once with resolved non-interactive flags:
   - baseline: `vegastack init --yes --json`
   - add `--scan` or `--no-scan`
...
Do not hand-write `.vegastack/vegastack.yml`, hooks, or VegaStack-managed entrypoint blocks. The CLI owns those files/blocks.
