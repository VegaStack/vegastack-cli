...
- **Backed by:** Cloudflare R2 bucket `vegastack-bundles` (E4 owns publishing).
- **Production URL:** `https://mcp.vegastack.com/mcp` (StreamableHTTP) and `https://mcp.vegastack.com/sse` (SSE) — DNS+route provisioned post-deploy.
...
wrangler r2 bucket create vegastack-bundles
wrangler r2 bucket create vegastack-bundles-preview
...
`wrangler.toml` declares an R2 binding `BUNDLE` pointing at the `vegastack-bundles` bucket. The worker reads:
