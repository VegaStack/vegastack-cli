...
|---|---|---|---|---|
| 1 | A3 | `vegastack skills install --agent all` only covered 4 agents; Continue/Aider/modern Gemini unreachable | `src/commands/skills.ts` and `src/commands/doctor.ts` swapped from `ALL_AGENT_NAMES`/`getAgent` → `ALL_RENDERER_NAMES`/`getRenderer`; `src/cli.ts` `VALID_AGENTS` now lists 6. `runSkills` and doctor's per-agent loop are now async (`Promise.all` over renderers). | ✅ |
...
- **Sigstore bundle verification in CLI** — `vegastack doctor --verify-attestations` documented but unimplemented; v0.2 work per E4's deliberate scope cut.
