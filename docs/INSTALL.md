...
`vegastack` is a thin npm-distributed loader (~200 KB) plus a content bundle
...
```
   ...
   runs npm/install.js (postinstall):
     1. Reads bundles.vegastack.com/manifest.json
        (latest channel by default; see VEGASTACK_BUNDLE_CHANNEL).
     ...
     5. Stages into ~/.config/vegastack/bundle.new, then atomically
        ...
        MoveFileEx + MOVEFILE_REPLACE_EXISTING on Windows.
              │
              ▼
   `vegastack` is on $PATH; `vegastack doctor` confirms everything.
```

If `postinstall` is blocked in your environment (some CI systems disable
it), set `VEGASTACK_SKIP_POSTINSTALL=1` and run `vegastack update` manually after
install to fetch the bundle.

## Verification: `vegastack doctor --verify-attestations`

`vegastack doctor` performs a fast health check (paths, bundle present,
...
Override with `VEGASTACK_BUNDLE_DIR`. The previous bundle generation is kept
...
`VEGASTACK_BUNDLE_CHANNEL=stable npm install -g @vegastack/cli` or
`vegastack update --channel stable`.

## Air-gapped install (`vegastack install --offline <path>`)
...
# On the target:
vegastack install --offline ./airgap-2026.07.01.tar.gz
```

`vegastack install --offline` will extract, verify SHA-256 + Sigstore against
a Fulcio root pinned in the CLI binary, and set `VEGASTACK_OFFLINE=1` so the
...
VEGASTACK_BUNDLE_URL="file:///path/to/vegastack-bundle-2026.04.28.tar.gz" \
  npm install -g @vegastack/cli
```
...
| Var | Effect |
|---|---|
| `VEGASTACK_BUNDLE_DIR` | Override the bundle install root. |
| `VEGASTACK_BUNDLE_URL` | Pull the full bundle from a custom URL (file:// works for offline). |
| `VEGASTACK_BUNDLE_MANIFEST_URL` | Override the manifest URL (default `https://bundles.vegastack.com/manifest.json`). |
| `VEGASTACK_BUNDLE_CHANNEL` | Channel to track: `latest` (default in v0.1). `stable` arrives v0.2. |
| `VEGASTACK_SKIP_POSTINSTALL` | Skip the postinstall bundle fetch. Useful in CI / dev. |
| `VEGASTACK_OFFLINE` | Disable all network calls (no update notifier, no manifest fetch). |
| `NO_UPDATE_NOTIFIER` / `VEGASTACK_NO_UPDATE_NOTIFIER` | Suppress the weekly "newer CLI available" nag. |
...
Every artifact `vegastack` consumes is verifiable:
...
Likely network or DNS. Re-run with `vegastack doctor` to see the exact URL
that failed. Common fixes:

- Corporate proxy strips Range requests → set `VEGASTACK_NO_RANGE=1` to fall
  back to a full-tarball download.
- Cloudflare R2 unreachable → the loader auto-falls-back to the GitHub
  Releases mirror; `vegastack doctor` will report "primary CDN unreachable,
  mirror used".
...
- The bundle was tampered with on disk → run `vegastack update --rollback`
  or reinstall.
- A daily bundle update raced your CLI install → run `vegastack update` to
